...
# load feature
featCityTrain = np.genfromtxt('./feature/training0.2-0.6.vector', delimiter = ' ');
featCityValid = np.genfromtxt('./feature/training0.2-0.6.vector', delimiter = ' ');

print featCityTrain.shape
print featCityTrain.shape


outlierId = np.genfromtxt('./outlier_index/OutlierIndex-50-0.5',dtype = np.int);
isOutlier = np.zeros( (featCityTrain.shape[0] ) );
isOutlier[outlierId] = 1;
isOutlier = np.bool_(isOutlier);

featCityTrain = np.delete(featCityTrain, \
            np.array(np.where(isOutlier == True) ), 0);
cityLabel = np.delete(cityLabel, np.array(np.where(isOutlier == True) ), 0);


featCountryTrain = np.genfromtxt('./feature/training0.1-0.6.vector', delimiter = ' ');
featCountryValid = np.genfromtxt('./feature/training0.1-0.6.vector', delimiter = ' ');
...
outFile = open('training50-0.5.txt','w');
